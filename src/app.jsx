...
class App extends React.Component {
	...
	getOpinion = response => {
	   // response pollid,name,selected options

		const { polls } = this.state;
		// find object using response id > from polls
		const poll = polls.find(p => p.id === response.pollId);
		// find option in poll or polls using response selectedoptions
		const option = poll.options.find(o => o.id === response.selectedOption);
		
		poll.totalVote++;
		option.vote++;
		// opinion object ta push poll a ,,,finallay polls setState
		const opinion = {
			id: shortid.generate(),
			name: response.name,
